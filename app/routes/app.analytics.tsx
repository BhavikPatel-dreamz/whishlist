...
export default function Analytics() {
  ...
  >("wishlists");
  const [open, setOpen] = useState(data.wStats.total === 0 && data.topVariants.length === 0);
  ...
  const [emailSubject, setEmailSubject] = useState(data.shop.emailSubject);
  const [emailHeading, setEmailHeading] = useState(data.shop.emailHeading);
  const [emailBody, setEmailBody] = useState(data.shop.emailBody);
  const [buttonLabel, setButtonLabel] = useState(data.shop.buttonLabel);
  const [stockAlertDeliveryMode, setStockAlertDeliveryMode] = useState(
    data.shop.stockAlertDeliveryMode,
  );
  const [stockAlertDeliveryTime, setStockAlertDeliveryTime] = useState(
    data.shop.stockAlertDeliveryTime,
  );
      ...
      <BlockStack gap="500">
        ...
        {/* <Card>
          <BlockStack gap="400">
            ...
            <Text as="p" tone="subdued">
              These settings control the email that is sent when a product comes back in stock.
            </Text>
            <Form method="post">
              <BlockStack gap="400">
                <TextField
                ...
                <Select
                  ...
                  options={[
                    { label: "Immediately when available in stock", value: "immediate" },
                    { label: "Given time in a day", value: "scheduled" },
                  ]}
                ...
                {stockAlertDeliveryMode === "scheduled" && (
                  <TextField
                    ...
                    helpText="This uses your store timezone."
                    autoComplete="off"
                  />
                )}
